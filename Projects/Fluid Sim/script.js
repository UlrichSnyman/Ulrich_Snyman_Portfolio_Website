...
function updateConfigUI() {
  document.getElementById('simRes').value = config.SIM_RESOLUTION;
  document.getElementById('simResValue').textContent = config.SIM_RESOLUTION;
  document.getElementById('dyeRes').value = config.DYE_RESOLUTION;
  document.getElementById('dyeResValue').textContent = config.DYE_RESOLUTION;
  document.getElementById('densityDissipation').value = config.DENSITY_DISSIPATION;
  document.getElementById('densityDissipationValue').textContent = config.DENSITY_DISSIPATION;
  document.getElementById('velocityDissipation').value = config.VELOCITY_DISSIPATION;
  document.getElementById('velocityDissipationValue').textContent = config.VELOCITY_DISSIPATION;
  document.getElementById('pressureDissipation').value = config.PRESSURE_DISSIPATION;
  document.getElementById('pressureDissipationValue').textContent = config.PRESSURE_DISSIPATION;
  document.getElementById('pressureIterations').value = config.PRESSURE_ITERATIONS;
  document.getElementById('pressureIterationsValue').textContent = config.PRESSURE_ITERATIONS;
  document.getElementById('curl').value = config.CURL;
  document.getElementById('curlValue').textContent = config.CURL;
  document.getElementById('splatRadius').value = config.SPLAT_RADIUS;
  document.getElementById('splatRadiusValue').textContent = config.SPLAT_RADIUS;
  document.getElementById('shading').checked = config.SHADING;
  document.getElementById('colorful').checked = config.COLORFUL;
  document.getElementById('paused').checked = config.PAUSED;
  document.getElementById('transparent').checked = config.TRANSPARENT;
  document.getElementById('bloom').checked = config.BLOOM;
  document.getElementById('bloomIterations').value = config.BLOOM_ITERATIONS;
  document.getElementById('bloomIterationsValue').textContent = config.BLOOM_ITERATIONS;
  document.getElementById('bloomResolution').value = config.BLOOM_RESOLUTION;
  document.getElementById('bloomResolutionValue').textContent = config.BLOOM_RESOLUTION;
  document.getElementById('bloomIntensity').value = config.BLOOM_INTENSITY;
  document.getElementById('bloomIntensityValue').textContent = config.BLOOM_INTENSITY;
  document.getElementById('bloomThreshold').value = config.BLOOM_THRESHOLD;
  document.getElementById('bloomThresholdValue').textContent = config.BLOOM_THRESHOLD;
  document.getElementById('bloomSoftKnee').value = config.BLOOM_SOFT_KNEE;
  document.getElementById('bloomSoftKneeValue').textContent = config.BLOOM_SOFT_KNEE;
  document.getElementById('backColorR').value = config.BACK_COLOR.r;
  document.getElementById('backColorG').value = config.BACK_COLOR.g;
  document.getElementById('backColorB').value = config.BACK_COLOR.b;
}

function setupConfigListeners() {
  const map = [
    ['simRes', 'SIM_RESOLUTION'],
    ['dyeRes', 'DYE_RESOLUTION'],
    ['densityDissipation', 'DENSITY_DISSIPATION'],
    ['velocityDissipation', 'VELOCITY_DISSIPATION'],
    ['pressureDissipation', 'PRESSURE_DISSIPATION'],
    ['pressureIterations', 'PRESSURE_ITERATIONS'],
    ['curl', 'CURL'],
    ['splatRadius', 'SPLAT_RADIUS'],
    ['bloomIterations', 'BLOOM_ITERATIONS'],
    ['bloomResolution', 'BLOOM_RESOLUTION'],
    ['bloomIntensity', 'BLOOM_INTENSITY'],
    ['bloomThreshold', 'BLOOM_THRESHOLD'],
    ['bloomSoftKnee', 'BLOOM_SOFT_KNEE']
  ];
  map.forEach(([id, key]) => {
    ...
    if (el) {
      el.addEventListener('input', e => {
        let v = el.type === 'range' ? (el.step.indexOf('.') > -1 ? parseFloat(el.value) : parseInt(el.value)) : el.value;
        config[key] = v;
  ...
  // Background color
  ['backColorR', 'backColorG', 'backColorB'].forEach((id, idx) => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('input', e => {
        config.BACK_COLOR[['r','g','b'][idx]] = parseInt(el.value);
      });
    }
  });
}
