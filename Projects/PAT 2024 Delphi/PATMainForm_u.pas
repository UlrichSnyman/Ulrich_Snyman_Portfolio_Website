...
////////////////////////////////////////////////////////////////////////////////
unit PATMainForm_u;
//TODO: Welcome user, Verify credit
interface
...
type
  ...
  private
    ...
    function VerifyBillInfo(const TabSheet: TTabSheet): Boolean;
  public
...
begin
  ...
  if mskedtPassword is TMaskEdit then
    TEdit(mskedtPassword).PasswordChar := '•';
  if mskedtPasswordReg is TMaskEdit then
    TEdit(mskedtPasswordReg).PasswordChar := '•';
  if mskedtPasswordVerifyReg is TMaskEdit then
    TEdit(mskedtPasswordVerifyReg).PasswordChar := '•';
end;
////////////////////////////////////////////////////////////////////////////////
...
begin
  // Bill logic is fake for obvious purposes
...
begin
  ...
  imgUserAvatar.Picture.LoadFromFile('DefaultProfile.jpg');
end;
...
begin
  lblWelcome.Caption := 'Welcome back, ' + FCurrentUser + '!';
...
procedure TfrmMain.SetCurrentUser(const Username: string);
begin
  FCurrentUser := Username;
  lblWelcome.Caption := 'Welcome back, ' + FCurrentUser + '!';
end;

function TfrmMain.VerifyBillInfo(const TabSheet: TTabSheet): Boolean;
var
  i: Integer;
  Control: TControl;
  ControlText: string;
begin
  ...
  begin
    Control := TabSheet.Controls[i];
    if (Control is TEdit) then
      ControlText := TEdit(Control).Text
    else if (Control is TMaskEdit) then
      ControlText := TMaskEdit(Control).Text
    else
      Continue; // Skip controls that are not TEdit or TMaskEdit

    if ControlText.Trim = '' then
    begin
...
begin
  ...
  SetCurrentUser(mskedtUsername.Text);
  PageControl1.ActivePage := tbsWelcome;
end;
...
var
  ...
  UserData: string;
begin
if VerifyBillInfo(tbsRegister) then
  begin
  ...
  SetCurrentUser(mskedtUsernameReg.Text);
  PageControl1.ActivePage := tbsWelcome;
